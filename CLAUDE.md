...
- **Quick windows** (`Domain/QuickWindows/`, `Application/QuickWindows/`, `Infrastructure/ModernProjects/QuickWindowStore.cs`) are a distinct entity from pages, persisted under `quick-windows/`. Since Phase 6, eleven of their thirteen runtime capabilities are `Supported` and strict 2.3 export carries them; `quick-window.binding.parent-port` and `quick-window.legacy-fragment-adapter` remain `Blocked`, and a project touching either is refused before any export directory exists. Known gap: the deployed title bar is empty because the compiler serialises the raw `PresentationDefaults.Title` instead of `EffectiveTitle` (`FR-UI-03`, gap 31).
