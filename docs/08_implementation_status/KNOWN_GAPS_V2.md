...
12. The full test suite currently reports 598 passed and 5 pre-existing unrelated failures (`Ft100ExportPrefersReferenceHtmlSourceBeforeRawFallback`, `LegacyContextMenuExposesElementStudioCommand`, `ModernDoubleClickOpensWpfPropertiesDialog`, `ReadOnlyNumericElementsRenderDisplayFormatWhenValueIsMissing`, `ScadaBuilderLaunchesStudioFromProjectInDevelopmentToAvoidStaleBinaries`). The targeted `DEC-0040` suites are green.
13. `DEC-0040` has implementation code and automated regressions, but its isolated Release UI gate is pending: record initial render for a 64 x 64 Table plus p95 cell-selection and track-resize feedback before declaring the product slice closed.
