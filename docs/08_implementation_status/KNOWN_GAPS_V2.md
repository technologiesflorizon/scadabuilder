...
18. Runtime coverage is not yet general. The typed capability registry and pure model analyzer now exist, but manifest negotiation, generated matrix, shared conformance `.sb2` and CI gate proving every page/binding/state/expression/effect/command/action variant remain pending. Current page-specific evidence must not be presented as full SCADA Builder V2 runtime conformance.
