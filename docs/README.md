...
| --- | --- | --- | --- |
| 2026-09-24 | `V2.1.6.0028` | `aef8dd0` | Tour 2 de revue du voile. Deux corrections d'honnêteté et une de mordant. Le verrou de `BusyOverlayController` sérialise les compteurs mais pas les rendus — le callback est appelé hors verrou, faute de quoi un rendu repassant par le dispatcher interbloquerait —, donc les `<remarks>` qui laissaient entendre une garantie de concurrence sont **corrigés** : le voile est affine au thread UI, le verrou protège les compteurs d'une corruption et le renvoi sur le dispatcher achète la sûreté d'un appel hors thread, pas son ordre. Séquencer les rendus aurait placé la logique dans `ApplyBusyOverlayState`, seul endroit de ce chantier qu'aucun test du dépôt ne peut atteindre, pour un chemin qu'aucun appelant livré n'emprunte. `State` se lit sous le même verrou que les deux profondeurs. Enfin, la garde de position du voile avait perdu du mordant au tour 1 : rien n'épinglait plus le décrément à l'intérieur de `LowerVeilOnce`, et un `try/finally` interne aurait satisfait la lecture du premier `finally`. Les deux trous sont fermés et vérifiés par mutation. Document propriétaire : `docs/06_ui_ux/UI_ARCHITECTURE_V2.md`. |
| 2026-09-24 | `V2.1.6.0027` | `ada1638` | Tour 1 de revue du voile de chargement. **La ligne `V2.1.6.0026` ci-dessous portait une affirmation fausse, corrigée ici et amendée là-bas** : le `finally` ne pouvait pas garantir que le voile soit sorti avant la boîte d'erreur, puisque `catch` s'exécute *avant* `finally`. La boîte modale s'affichait donc devant l'anneau en rotation jusqu'à ce que l'opérateur la ferme. Chaque `catch` baisse maintenant le voile lui-même, de façon idempotente, le `finally` restant la garantie. Le gel couvre aussi le clavier, conformément à la demande d'origine (« empêcher de tenter 2 actions en même temps ») : un bouton d'accueil ayant le focus se redéclenchait sur Entrée et `Ctrl+Z`/`Ctrl+Y` atteignaient l'éditeur. Quatre dialogues suspendent désormais le voile au lieu de deux (s'ajoutent la création de projet et les modifications non enregistrées), aucun message d'échec n'est plus présenté sous le voile, `Suspend()` ne fuit plus si le rendu lève, et les compteurs sont verrouillés et le rendu renvoyé sur le dispatcher. Document propriétaire : `docs/06_ui_ux/UI_ARCHITECTURE_V2.md`. |
| 2026-09-24 | `V2.1.6.0026` | `21ff714` | Voile de chargement sur les gestes projet : cliquer « Rouvrir *projet* » ou « Ouvrir » ne donnait aucun signe que le chargement avançait, et rien n'empêchait de lancer une deuxième action par-dessus. Un voile couvre désormais toute la fenêtre pendant un geste — anneau animé en XAML pur, libellé du geste, nom du projet quand il est connu — et intercepte les clics, ce qui gèle ruban, panneaux et canvas d'un seul geste. Il monte dans le `try` de `RunProjectGestureAsync` et son `finally` garantit qu'il redescend, donc un échec ne laisse pas la fenêtre morte; il se retire le temps des questions posées à l'intérieur d'un geste. Document propriétaire : `docs/06_ui_ux/UI_ARCHITECTURE_V2.md`. |
| 2026-09-12 | `V2.1.6.0025` | `705b13c` | `ElementPropertiesDialog` peut désormais exprimer un **fond transparent** : une case « Transparent » est ajoutée à la couleur d'arrière-plan, sur le modèle exact de celle de la bordure. Le modèle et l'export acceptaient `"Transparent"` depuis toujours (un groupe est créé avec), mais le dialogue imposait une couleur concrète, `#FFFFFF` par défaut. C'est la façon de créer une zone invisible qui porte des évènements — un clic sur une région du synoptique, par exemple. |
| 2026-09-12 | `V2.1.6.0024` | `b4ca385` | Correctif de test humain : `UnsavedChangesDialog` et `ConversionPlanDialog` posaient `Background="{StaticResource PanelBrush}"` sur l'élément `Window` alors que la clé est définie plus bas dans leur propre `Window.Resources`. XAML résout `StaticResource` à l'analyse en ne regardant que vers l'arrière, donc les deux fenêtres levaient au chargement et ne pouvaient pas s'ouvrir. La brosse descend sur l'élément enfant racine; un contrat de source balaie désormais tout `src/*.xaml` pour ce motif. |
