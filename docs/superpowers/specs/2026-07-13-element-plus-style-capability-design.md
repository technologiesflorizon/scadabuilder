...
Version du document: `V2.1.3.0008`
...
Les anciens JSON n'ont aucun des nouveaux champs. La désérialisation `System.Text.Json` avec `JsonSerializerDefaults.Web` (utilisée par `ModernProjectStore`) obtiendra les défauts du record. Aucune migration destructive n'est exécutée.
...
Ce JSON se désérialise correctement : toutes les nouvelles propriétés reçoivent leur défaut et le rendu est identique à l'historique.
...
- Les nouveaux champs ne sont sérialisés que lorsqu'ils diffèrent du défaut (comportement standard de `System.Text.Json` avec `DefaultIgnoreCondition.WhenWritingDefault` — à vérifier dans `ModernProjectStore`).
...
- Les projets ouverts avec une ancienne version de SCADA Builder V2 ignoreront les nouveaux champs JSON (comportement standard de `System.Text.Json` avec `JsonIgnoreCondition.WhenWritingDefault` ou équivalent).
...
**Q1 — Aperçu vivant (live preview).** L'aperçu local dans le dialogue modal (un `Border` ou `TextBlock` WPF reflétant le style en cours d'édition) est-il obligatoire dès la première itération, ou peut-il être traité comme une phase UI séparée après la livraison des propriétés structurées et de la refonte des sections ?

- **Option A** : Obligatoire dans la première itération. L'aperçu fait partie de la direction d'interaction D17 et constitue un critère d'acceptation implicite.
- **Option B** : Phase séparée. Livrer d'abord les propriétés model-backed + sections + icônes + rendu preview/export, puis ajouter l'aperçu local WPF dans une itération suivante. L'avantage est de réduire le risque UI et de valider le modèle et le rendu avant d'investir dans l'aperçu.
