...
1. **`FR-UI-03` — barre de titre vide dans le runtime déployé.** Le compilateur sérialise `PresentationDefaults.Title` brut, alors que `QuickWindowPresentationDefaults.EffectiveTitle` implémente le repli sur le `DisplayName` et que le compilateur l'applique déjà à la scène. L'aperçu Builder affiche donc le nom, le runtime déployé n'affiche rien. Report délibéré, hors Phase 6, consigné en entrée 31 de `KNOWN_GAPS_V2.md`. La traçabilité de `FR-UI-03` est verte — la FR est portée par une tâche testée — mais son comportement déployé ne l'est pas.
