...
| --- | --- | --- | --- |
| 2026-07-15 | `V2.1.4.0034` | `b75f1d7` | Tranches correctives implementees et validees; contrats source ajustes dans `c441090`. |
...
- [x] Verifier que la specification corrective est `Approved` et que sa nouvelle decision est enregistree.
...
- [x] Capturer un baseline frais avant modification.
...
- [x] Confirmer les causes sur le commit de depart.
...
- [x] Ecrire d'abord des tests qui construisent un element verrouille, un element deverrouille et un groupe avec enfant verrouille; serialiser le payload reel et verifier chaque valeur `IsLocked`.
- [x] Creer le DTO dedie et `ModernElementRenderPayloadFactory.Create(ScadaElement, IReadOnlySet<string>, int)` en deplacant la projection actuellement privee dans `MainWindow`.
- [x] Projeter `IsLocked` recursivement pour racines et descendants, sans calculer un verrou effectif different du modele.
- [x] Remplacer `ToRenderPayload` par un appel de factory et retirer le type correspondant de `MainWindow.NestedTypes.cs`.
- [x] Garder `data-editor-locked` editor-only; ajouter une regression qui prouve que le script le lit depuis le payload reel, pas seulement qu'une chaine JavaScript existe.
- [x] Executer :
...
- [x] Ajouter des tests rouges d'ordre qui exigent le refus avant creation de `modernDrag` et avant pointer capture pour move, resize nord/ouest et resize de groupe avec descendant verrouille.
- [x] Centraliser dans le script les predicates `blocksPositionMove`, `isPositionChangingResize` et `containsLockedDescendant`.
- [x] Refuser tout move si une cible effective est verrouillee.
- [x] Pour un objet simple verrouille, permettre seulement les handles est, sud et sud-est; refuser les handles qui exigent un changement de X/Y.
- [x] Refuser avant apercu un resize de groupe si le groupe ou un descendant verrouille changerait de position.
- [x] Stocker `positionLocked` dans l'etat de drag autorise et forcer X/Y aux valeurs initiales pendant chaque preview de resize comme defense complementaire.
- [x] Verifier que la rotation conserve les X/Y du modele et que le guard Application demeure appele au commit.
- [x] Etendre `ElementTransformGuardTests` uniquement pour les cas contractuels manquants : W/H a X/Y fixes accepte, X/Y modifie refuse, groupe avec descendant deplace refuse.
- [x] Executer :
...
- [x] Ecrire les tests rouges de la matrice : placement -> Objet; deselection -> Objet; reselection -> Objet; passage explicite Cellules; A/1 depuis Objet -> Cellules visible; masquer en Cellules; retour Objet -> invisible.
- [x] Modifier `CompletePlacement` pour terminer en mode Objet.
- [x] Faire de `SelectTable` une transition explicite : nouvelle selection ou reselection apres absence -> Objet; une simple actualisation du meme id ne doit pas casser un mode Cellules actif.
- [x] Ajouter `EditorGuidesVisible => Mode == Cells && ShowEditorGuides`.
- [x] Faire entrer `ToggleEditorGuides` en mode Cellules avec guides actifs lorsqu'elle est invoquee depuis Objet; en mode Cellules, basculer seulement la preference.
- [x] Faire utiliser `EditorGuidesVisible` par `TableRibbonStateProvider.Build` pour libelle, pressed state et aide.
- [x] Verifier qu'aucune mutation de lock n'appelle `SetMode` ou ne derive le mode depuis `IsLocked`.
- [x] Executer :
...
- [x] Tester les quatre couples Objet/Cellules x preference A/1 et la serialisation JavaScript echappee.
- [x] Creer le record et la factory stateless; ne pas exposer le modele mutable.
- [x] Ajouter `window.scadaModernTable.setEditorState(mode, guides)` qui applique mode et visibilite effective dans le meme tour JavaScript.
- [x] Remplacer les envois separes par `SyncTableEditorStateInWebViewAsync` dans `MainWindow.TableIntegration.cs`.
- [x] Appeler la synchronisation apres creation, selection/deselection, commande Objet/Cellules, commande A/1, fermeture de surface et rerender apres lock.
- [x] Garantir que le lock ne force jamais Objet ou Cellules.
- [x] Verifier les priorites de pointeur : en Cells, cellule/en-tete/separateur consomme l'evenement avant le drag Element+; en Object, le corps du Tableau appartient au wrapper.
- [x] Verifier double-clic et Escape, y compris avec `data-editor-locked=true`.
- [x] Maintenir `MainWindow` au niveau delegation seulement; le test d'architecture doit refuser le retour de calculs inline de mode/guides.
- [x] Executer :
...
- [x] Renforcer les regressions d'export : absence de `data-editor-locked`, DTO editor-only, A/1, handles et overlays dans `.sb2` et `.sep`.
- [x] Executer build et tests cibles :
...
- [x] Executer la suite complete et comparer au baseline frais :

```powershell
dotnet test ScadaBuilderV2.sln --no-restore
```

- [x] Executer le smoke interactif exact de la section 7.2 de la specification sur une copie isolee. Consigner resultat, date, branche/commit et toute divergence.
- [x] Inspecter une archive `.sb2` et un `.sep` produits par le smoke.
- [x] Enregistrer la nouvelle decision; ne pas modifier `DEC-0040`, mais indiquer que la nouvelle decision remplace ses seuls comportements d'interaction explicitement listes.
- [x] Synchroniser les contrats proprietaires, couverture, statut et gaps. Marquer spec/plan `Implemented` seulement apres le smoke reussi.
- [x] Appliquer le bump requis depuis la version presente au moment de la cloture; ne pas reutiliser aveuglement `V2.1.4.0032`.
- [x] Executer :
...
- [x] Specification corrective approuvee et nouvelle decision enregistree sans modifier `DEC-0040`.
- [x] Le payload editor-only transporte `IsLocked` recursivement et un test serialise le DTO reel.
- [x] Aucun drag verrouille ne demarre ou ne deplace visuellement le wrapper.
- [x] Les handles de resize verrouille respectent X/Y invariants; les transformations de groupe interdites n'ont pas d'apercu.
- [x] `ElementTransformGuard` reste la defense finale contre les messages forges ou chemins secondaires.
- [x] Un Tableau neuf termine en mode Objet et se deplace s'il est deverrouille.
- [x] Deselection/reselection, Objet/Cellules, double-clic et Escape sont deterministes.
- [x] Le verrou ne change jamais le mode Tableau.
- [x] Cellules, headers et separateurs restent utilisables sur un Tableau verrouille en mode Cellules.
- [x] `Afficher/Masquer A/1` reflete `EditorGuidesVisible` et agit immediatement.
- [x] Mode et guides sont synchronises dans une seule operation WebView.
- [x] Aucun calcul de politique n'est ajoute a `MainWindow`.
- [x] `.sb2`, `.sep` et TF100Web restent inchanges.
- [x] Build, tests cibles, suite complete, docs verifier et smoke interactif sont consignes.
- [x] Aucun fichier sous `projects/AMR_REF_SCADA_V2` n'est modifie ou stage.
